...
(function ($, Drupal) {
  ...
  Drupal.behaviors.bootstrapAnchors = {
    ...
    bootstrapAnchor: function (element) {
      element.validAnchor =
        element.nodeName === 'A' &&
        (location.hostname === element.hostname || !element.hostname) &&
        element.hash.replace(/#/, '').length;
      element.scrollTo = function (event) {
        ...
        var offset =
          $target.offset().top -
          parseInt($scrollableElement.css('paddingTop'), 10) -
          parseInt($scrollableElement.css('marginTop'), 10);
        if (this.validAnchor && $target.length && offset > 0) {
          ...
          var $fakeAnchor = $('<div/>')
            ...
            .css({ position: 'absolute', top: offset + 'px', zIndex: -1000 })
            .appendTo(document);
